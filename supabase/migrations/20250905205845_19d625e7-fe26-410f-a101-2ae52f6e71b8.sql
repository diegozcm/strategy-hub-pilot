-- Remove problematic functions that try to create auth users directly
DROP FUNCTION IF EXISTS public.create_user_by_admin_v2(uuid, text, text, text, text, text, text, text, app_role, uuid);
DROP FUNCTION IF EXISTS public.create_user_by_admin(uuid, text, text, text, text, text, text, text, app_role, uuid);

-- Create simplified function to configure user profile after auth user creation
...
BEGIN
  ...
  BEGIN
    ...
    IF _company_id IS NOT NULL THEN
      ...
      VALUES (_user_id, _company_id, _role::VARCHAR, NOW(), NOW())
