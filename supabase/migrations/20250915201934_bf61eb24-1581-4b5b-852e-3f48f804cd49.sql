-- Fix the analyze_user_data function with correct table names
...
BEGIN
  -- Get strategic projects data (correct table name)
  ...
  -- Get key results data (correct table name)
  SELECT COALESCE(json_agg(
    json_build_object(
      'id', kr.id,
      'name', kr.title,
  ...
  -- Get strategic objectives data (correct table name)
  SELECT COALESCE(json_agg(
    json_build_object(
      'id', o.id,
      'plan_id', o.plan_id,
