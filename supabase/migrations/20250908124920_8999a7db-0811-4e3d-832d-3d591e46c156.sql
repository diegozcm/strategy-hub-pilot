-- Fix type compatibility in find_compatible_replacement_users function
    ...
    (
      ...
      CASE WHEN (
        ...
        WHERE um2.user_id = p.user_id 
          AND um2.module_id = ANY(COALESCE(target_user_modules, ARRAY[]::uuid[])) 
          AND um2.active = true
      ...
      CASE WHEN (
        ...
        WHERE ucr2.user_id = p.user_id 
          AND ucr2.company_id = ANY(COALESCE(target_companies, ARRAY[]::uuid[]))
    ...
    json_build_object(
      ...
      'compatible_modules', (
        ...
        WHERE um.user_id = p.user_id 
          AND um.module_id = ANY(COALESCE(target_user_modules, ARRAY[]::uuid[])) 
          AND um.active = true
      ),
      'shared_companies', (
        ...
        WHERE ucr.user_id = p.user_id 
          AND ucr.company_id = ANY(COALESCE(target_companies, ARRAY[]::uuid[]))
      ),
