-- Fix: KRs with target_direction='minimize' and no data (actual=0) should show 0% instead of 100%
-- This was causing inflated progress calculations for pillars and objectives
...
BEGIN
  ...
  -- ====================================================================
  -- Calculate yearly values based on ALL 12 months and aggregation_type
  -- ====================================================================
  ...
  ELSIF kr.target_direction = 'minimize' THEN
    -- FIX: For minimize KRs, only calculate percentage if actual > 0
    -- If actual = 0, it means no data, so percentage should be 0 (not 100)
    
    -- Q1
    IF q1_actual_val > 0 AND q1_target_val > 0 THEN
      q1_percentage_val := ROUND((q1_target_val / q1_actual_val) * 100, 2);
    ELSE
      -- No actual data or no target = 0% progress (cannot measure)
      q1_percentage_val := 0;
    END IF;
    
    -- Q2
    IF q2_actual_val > 0 AND q2_target_val > 0 THEN
    ...
    -- Q3
    IF q3_actual_val > 0 AND q3_target_val > 0 THEN
    ...
    -- Q4
    IF q4_actual_val > 0 AND q4_target_val > 0 THEN
    ...
    -- Monthly
    IF month_actual_val > 0 AND month_target_val > 0 THEN
    ...
    -- YTD
    IF ytd_actual_val > 0 AND ytd_target_val > 0 THEN
    ...
    -- Yearly
    IF year_actual_val > 0 AND year_target_val > 0 THEN
...
-- Recalculate all existing KRs to apply the fix
DO $$
DECLARE
  kr_record RECORD;
BEGIN
  FOR kr_record IN SELECT id FROM key_results WHERE target_direction = 'minimize' LOOP
