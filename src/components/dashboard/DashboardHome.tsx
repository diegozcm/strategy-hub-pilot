...
export const DashboardHome: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const {
        data: plansData
      } = await supabase.from('strategic_plans').select('id').eq('company_id', company.id);
      ...
      } = await supabase.from('strategic_objectives').select(`
          id,
          title,
          description,
          monthly_targets,
      ...
      const {
        data: projectsData
      } = await supabase.from('strategic_projects').select('id, status').eq('company_id', company.id);

      // Buscar KRs dos objetivos com dados mensais
      const objectiveIds = objectivesData?.map(obj => obj.id) || [];
      const {
        data: keyResultsData
      } = await supabase.from('key_results').select(`
          id, 
          objective_id,
          title,
          status, 
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {getDynamicStats(dashboardStats).map(stat => <Card key={stat.title} className="hover:shadow-lg transition-shadow">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center mt-2">
                    {stat.changeType === 'positive' ? <ArrowUp className="h-3 w-3 text-green-500 mr-1" /> : stat.changeType === 'negative' ? <ArrowDown className="h-3 w-3 text-red-500 mr-1" /> : <div className="w-3 h-3 mr-1" />}
                    <span className={`text-xs font-medium ${stat.changeType === 'positive' ? 'text-green-600' : stat.changeType === 'negative' ? 'text-red-600' : 'text-gray-600'}`}>
                      {stat.change}
                    </span>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <div>
          <Card>
            ...
            <CardContent>
              {loading ? <div className="text-center py-4">Carregando objetivos...</div> : objectives.length === 0 ? <div className="text-center py-8">
                ...
                return <div key={objective.id} className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                ...
                                {months.map(month => <TableHead key={month.key} className="text-center min-w-20">
                                    {month.name}
                            ...
                            return <TableCell key={month.key} className={`text-center ${isCurrentMonth ? "bg-blue-50" : ""}`}>
                                      {performance.target || '-'}
                                    </TableCell>;
                          })}
                              </TableRow>
                            ...
                            const isCurrentMonth = month.key === currentMonth;
                            return <TableCell key={month.key} className={`text-center ${isCurrentMonth ? "bg-blue-50" : ""}`}>
                                      {performance.target > 0 ? <div className="flex justify-center">
